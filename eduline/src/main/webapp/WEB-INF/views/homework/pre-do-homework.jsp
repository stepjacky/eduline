...
<table class="table table-bordered table-hover" id="answerCard">
    <caption>
        <button type="button" class="close unblock" aria-label="Close"><span aria-hidden="true">&times;</span></button>
    </caption>
    <tbody>
    <c:forEach items="${modelAnswers}" var="item" varStatus="vs">
        ...
        <td>
            <div class="serial">${vs.index+1}</div>
        </td>
        <td class="answer-${vs.index+1}-td answer-td" align="left">

            <label class="btn-label"><input type="radio" class="answer" data-key="${vs.index}"
                                            name="na${item}me${vs.index}" value="A">
                [A]</label>
            <label class="btn-label"><input type="radio" class="answer" data-key="${vs.index}"
                                            name="na${item}me${vs.index}" value="B">
                [B]</label>
            <label class="btn-label"><input type="radio" class="answer" data-key="${vs.index}"
                                            name="na${item}me${vs.index}" value="C">
                [C]</label>
            <label class="btn-label"><input type="radio" class="answer" data-key="${vs.index}"
                                            name="na${item}me${vs.index}" value="D">
                [D]</label>
...
<button type="button" class="btn btn-primary btn-lg btn-block submitHomework"><i class="fa fa-hand-o-up"
                                                                                 aria-hidden="true"></i>交卷
</button>
