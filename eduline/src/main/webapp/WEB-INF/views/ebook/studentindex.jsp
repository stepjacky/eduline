...
<ol class="breadcrumb-ext">
  <li><a class="active" href="/ebook/student/0?ajax=false">
 ...
 <c:forEach items="${tags}" var="tag">
  <li>
   <a href="/ebook/with/tags/0/10?tags=${tag}">
			...
			<c:forEach items="${pager.dataList}" var="item" varStatus="vs">
				<c:if test="${vs.index%4==0}">
				  <tr>
					...
					src="/mycover/large/${(fn:endsWith(item.coverPath,'.htm') || fn:endsWith(item.coverPath,'.html'))?'404.jpg':item.coverPath}" style="width:80px;height:100px">
					</a>
					</p>
					...
					<c:forTokens items="${item.tags}" delims="," var="tag">
					   	...
					   	<a class="label label-info " href="/ebook/with/tags/0/10?tags=${tag}">
    							${tag}
   						</a>
